...
export function mapProvRequests(items) {
  return (items || []).map((it) => ({
    ...
    managerUpn: (it.ManagerUpn || '').toLowerCase(),
    deptManagerName: it.DeptManagerName || '',
    deptManagerUpn: (it.DeptManagerUpn || '').toLowerCase(),
    unitManagerName: it.UnitManagerName || '',
    unitManagerUpn: (it.UnitManagerUpn || '').toLowerCase(),
